...
def hard_bootstrap(G, nrounds=5, **kwargs):
    N = len(G)
    # random init
    labels = np.random.binomial(1,0.5,size=N)
    vfixos = np.zeros(N,dtype=int)
...
def soft_bootstrap(G, nrounds=5, **kwargs):
    N = len(G)
    # random init
    labels = np.random.binomial(1,0.5,size=N)
    vfixos = np.zeros(N,dtype=int)
...
def bootstrap_method(G, restart, nrounds=5, **kwargs):
    if restart == 'hard':
        return hard_bootstrap(G,nrounds,**kwargs)
    elif restart == 'soft':
        return soft_bootstrap(G,nrounds,**kwargs)
    else:
