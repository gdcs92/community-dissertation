...
algorithms = ['mva','gam'] #,'hard'] #'spectral' 
columns = ['acuracia', 'niter'] # 'niter'
...
for c in columns:
    ...
    fig.legend(loc='lower left', bbox_to_anchor=(0.2,0.7), fontsize=13)
    fig.tight_layout()

    if save:
        file_location = './figures/'
        ps = f'{p:.4f}'[2:]
        file_name = f'g1_n{n:d}_p{ps:s}_R{R:d}_M{M:d}_s{sd:d}_{c:s}.png'
