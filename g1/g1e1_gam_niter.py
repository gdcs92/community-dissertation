...
#    for j,a in enumerate(algorithms):
    ...
    if c == 'onelabels':
        if p == 0.02:
            fig.legend(loc='lower left', bbox_to_anchor=(0.15,0.3), fontsize=13)
        if p == 0.05:
            fig.legend(loc='lower left', bbox_to_anchor=(0.15,0.15), fontsize=13)
    else:
    ...
    if save:
        ...
        ps = f'{p:.4f}'[2:]
        file_name = f'g1_gam_n{n:d}_p{ps:s}_s{sd:d}_{c:s}.png'
