...
algorithms = ['spectral','mva','gam','hard','soft']
columns = ['acuracia']
...
#    for j,a in enumerate(algorithms):
    ...
    ax.set_xlim(right=p)
    if c in ['acuracia','pvfixos','acvfixos','acvmudam']:
        ax.set_ylim(top=1.03)
    if p == 0.02:
        ax.set_xticks(np.linspace(0,p,num=6))

    if p == 0.05:
        fig.legend(loc='lower left', bbox_to_anchor=(0.54,0.46), fontsize=13)
    else:
        fig.legend(loc='lower left', bbox_to_anchor=(0.15,0.64), fontsize=13)
    ...
    if save:
        ...
        ps = f'{p:.4f}'[2:]
        file_name = f'g1_bs_n{n:d}_p{ps:s}_s{sd:d}_{c:s}.png'
